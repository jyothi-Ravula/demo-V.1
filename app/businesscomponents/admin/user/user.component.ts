...
export class UserComponent implements OnInit {
	...
	//Drop down for departmentArray1.
	public departmentArray1 = [
		{ "deptID": 1, "name": "Accounts" },
		{ "deptID": 2, "name": "Management" },
		{ "deptID": 3, "name": "HR" },
		{ "deptID": 4, "name": "Finance" },
		{ "deptID": 5, "name": "Role" },
	];

	 //Drop down for search options.
	public locationArray1 = [
		{ "locationId": 1, "description": "Duluth" },
		{ "locationId": 2, "description": "Management" },
	...
	createObj() {
		...
		user.locationID = (user.locationID === null || user.locationID === "" || user.locationID === undefined) ? 0 : user.locationID;
	...
	populateData(row) {
		...
		this.departmentArray1.forEach(item => {
			if (item.deptID === row.departmentId) {
				this.userForm.controls['departmentId'].setValue(item.deptID);
			}
		});
		this.locationArray1.forEach(loc => {
