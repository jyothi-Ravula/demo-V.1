...
export class PratesComponent implements OnInit {
	...
	onFormSubmit() {
		...
		prate.userName = this.userName.nativeElement.value;
	...
	ValidatepRate(pRates: any): boolean {
		...
		this.dataSource.data.forEach(item => {
			if (this.IdToUpdate == null) {
				if (item.description.toLowerCase() === pRates.description.toLowerCase() && item.active === pRates.active) {
					this.isValid = false;
				}
			}
			else {
	...
	ResetChanges() {
		this.prateForm.controls['typeCode'].setValue("");
		this.prateForm.controls['rate'].setValue("");
		this.prateForm.controls['startDate'].setValue(null);
		this.prateForm.controls['startDateWithPicker'].setValue(null);
		this.prateForm.controls['displayOrder'].setValue("");
		this.prateForm.controls['description'].setValue("");
		this.prateForm.controls['userName'].setValue({'userName':''});
		...
		this.setSaveBtnEnable();
		this.prateForm.controls['active'].setValue(true);
		this.setFocus();
		Object.keys(this.prateForm.controls).forEach(key => {
			this.prateForm.controls[key].setErrors(null)
		});
	}
	...
	loadPRatesToEdit(row) {
		...
		this.userName.nativeElement.value = row.userName;
		this.setSaveBtnEnable();
	}
	...
	setDateInDatePicker(controlName1, controlName2) {
		...
		if (dateToSet == "Invalid Date" || this.prateForm.controls[controlName1].value == null || this.prateForm.controls[controlName1].value == ""){/*  || dateToSet<this.prateForm.get('startDate').value */
	...
	validateDecimals(key) {
	  ...
	  //load UserList
	loadUserList() {
		 this.userList = this.userScreenService.getAllUsers();
		this.userList.subscribe(results => {
			this.userNames = results;
			this.userNames.sort((a, b) => (a.userName > b.userName) ? 1 : ((b.userName > a.userName) ? -1 : 0));
			this.initializeAutocompleteUser();
		}); 
	}

	//to initialize user name and code to auto complete
	initializeAutocompleteUser() {
		this.filteredOptionsUser = this.prateForm.controls['userName'].valueChanges
			.pipe(
				startWith<string | any>(''),
				map(value => typeof value === 'string' ? value : value.userName),
				map(userName => userName ? this._filter(userName) : this.userNames.slice())
			);
	}
	...
	private _filter(name: string): any[] {
		...
		return this.userNames.filter(option => option.userName.toLowerCase().indexOf(filterValue) === 0);
	}


	  
	//Validation for userName AutoComplete.
	validateUserAutoCompletes(event) {
		let  filterUser : any = (event.target.value != null && this.userNames.length > 0) ? 
		 (this.userNames.find(cst => cst.userName.toLowerCase() === event.target.value.toLowerCase()) != undefined ? true : false)
		 : false;
		if (event.target.value =='') {
			...
			if(!filterUser){
				this.prateForm.controls['userName'].setErrors({'incorrect': true});
			}
			else{
				this.prateForm.controls['userName'].setErrors(null);
			}
		}
	}
